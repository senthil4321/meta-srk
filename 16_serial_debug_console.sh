...
1. Prerequisites on Host:
   ...
   - vmlinux-debug symbol file available

2. Enable KGDB on Target:
   # Boot into debug kernel first
   echo g > /proc/sysrq-trigger     # Enter KGDB mode
   # OR
   echo 1 > /proc/sys/kernel/sysrq  # Enable SysRq
   # Then trigger via console

3. Connect GDB on Host:
