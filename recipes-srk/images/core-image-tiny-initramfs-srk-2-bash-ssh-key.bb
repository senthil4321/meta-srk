SUMMARY = "Tiny image with bash shell and SSH key-based authentication support."
DESCRIPTION = "Tiny image with bash shell and SSH support (password + key-based authentication) capable of booting a device."
...
install_ssh_keys() {
    # Install SSH public keys for key-based authentication
    ...
    if [ -d "${IMAGE_ROOTFS}/root/.ssh" ]; then
        ...
        bbwarn "Root .ssh directory not found, skipping key installation for root"
    fi
    ...
    if [ -d "${IMAGE_ROOTFS}/home/srk/.ssh" ]; then
        ...
        bbwarn "SRK .ssh directory not found, skipping key installation for srk"
    fi
