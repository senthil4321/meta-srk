...
# Copy the boot files
cp "$SOURCE_DIR"/MLO /mnt/beaglebone-boot/
cp "$SOURCE_DIR"/u-boot.img /mnt/beaglebone-boot/
cp "$SOURCE_DIR"/zImage /mnt/beaglebone-boot/
cp "$SOURCE_DIR"/am335x-boneblack.dtb /mnt/beaglebone-boot/

# Copy the root filesystem
tar -xvf "$SOURCE_DIR"/beaglebone-yocto-rootfs.tar.gz -C /mnt/beaglebone-rootfs/
